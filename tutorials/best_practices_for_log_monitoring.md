...
本教程需要在监控服务器提前部署 Grafana，具体安装方法可以参考[集群运维监控](cluster_monitor.md)中软件安装部署的部分。

## 3. Loki 安装、部署、应用流程

注：

在流程开始前建议预先构建具有多数据节点 DolphinDB 高可用集群。具体安装方法可以参考[高可用集群部署与升级](ha_cluster_deployment.md)。也可以参考[基于 Docker-Compose 部署
DolphinDB](docker-compose_high_cluster.md)。
...
* **配置 Grafana 的 Loki 数据源：**
  ...
  根据[集群运维监控](cluster_monitor.md)中软件安装部署中 Grafana 部分的配置，Grafana
...
[集群运维监控](cluster_monitor.md)

[高可用集群部署与升级](ha_cluster_deployment.md)
