...
提供了一系列最优化函数，包括 [linprog](../funcs/l/linprog.md)、[quadprog](../funcs/q/quadprog.md)、[qclp](../funcs/q/qclp.md)、[socp](../funcs/s/socp.md)，帮助用户将业务问题转化为数学规划模型，并利用合适的求解器寻求最优解或可行解，从而辅助做出更明智的决策。
...
[socp](../funcs/s/socp.md) 函数用于求解二阶锥规划（SOCP, Second Order Cone
...
Gx + s = h 包含了 SOCP 问题中所有的不等式约束条件，包括线性不等式以及二阶锥约束。[socp](../funcs/s/socp.md) 函数中的参数 l 表示线性不等式约束个数的维度的整数标量；q
...
线性规划问题为线性约束条件下的线性目标函数的最优化问题，可以直接通过 DolphinDB 中的 [linprog](../funcs/l/linprog.md)
...
二次规划问题为线性约束条件下的二次目标函数的最优化问题，可以直接通过 DolphinDB 中的 [quadprog](../funcs/q/quadprog.md) 或转换为二阶锥问题进行求解。原问题形式为：
