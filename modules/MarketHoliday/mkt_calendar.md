...
* **在线更新**
  ...
  + 首先，下载 [marketHoliday](https://gitee.com/dolphindb/DolphinDBModules/tree/master/MarketHoliday/marketHoliday) 文件夹。
  + 然后，将该文件夹存放至和 dolphindb 同一服务器的任意位置。注意：若为集群环境，每台机器都需要放置该文件夹。同时，为了验证是否更新成功，请不要保留原本 <dolphindb>/server 路径下的 marketHoliday 文件夹。替换完成后，参照以下代码实现在线更新（以更新 2025 年交易日历为例）：
  ...
  其中，`update_2025_holiday` 函数仅在当前节点有效。在集群环境中，可通过 `pnodeRun` 调用该函数，使更新操作在其它节点生效。
